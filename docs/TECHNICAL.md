...
This document preserves the detailed implementation, configuration, wire
protocol, consistency, security, monitoring and benchmark reference. Start
with the concise [README](../README.md); use the
[existing-database guide](INSTALL_EXISTING.md) for a production installation.
...
Зафиксированный long-run reference текущей методики дал следующие median
throughput values:
...
invalidation. Это reference baseline, а не capacity promise. Текущий
проверяемый CI smoke, параметры запусков и правила замены baseline приведены
в [отдельном документе о бенчмарках](BENCHMARKS.md).
