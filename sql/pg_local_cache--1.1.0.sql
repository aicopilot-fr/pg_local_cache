...
CREATE TABLE mapping (
    ...
    /* 1.0 compatibility projection, maintained by the registration API. */
    key_column name,
    value_column name,
    writable boolean NOT NULL DEFAULT false,
    /* Keep the physical order identical to a 1.0 -> 1.1 upgrade. */
    ...
    CONSTRAINT mapping_key_column_projection CHECK (
...
/* Kept separate so the stable 1.0 metrics() row type remains upgrade-safe. */
...
/* Exact 1.0 registration signature, retained as a scalar compatibility shim. */
CREATE FUNCTION register_mapping(
    p_namespace text,
    p_relation regclass,
    p_key_column name,
...
/* Private catalog anchor for dependencies dumped after a 1.0 -> 1.1 upgrade. */
CREATE FUNCTION _attach_table_1_0_compat(
    p_relation regclass,
...
/* Preserve the 1.0 two-, three-, and four-argument scalar call shapes. */
CREATE FUNCTION attach_table(
    p_relation regclass,
    p_value_column name,
    p_namespace text DEFAULT NULL,
